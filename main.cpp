...
main(int ac, const char* av[])
{
    ...
    if (testnet && deamon_url == "http:://127.0.0.1:38081")
        deamon_url = "http:://127.0.0.1:48081";
    if (stagenet && deamon_url == "http:://127.0.0.1:38081")
        deamon_url = "http:://127.0.0.1:58081";
    ...
    ([&](const crow::request& req)
     {
        ...
        {
            return string("msr address, viewkey or tx hash not provided");
        }
    ...
    CROW_ROUTE(app, "/prove").methods("POST"_method)
        ([&](const crow::request& req) {
            ...
            {
                return string("msr address, tx private key or "
